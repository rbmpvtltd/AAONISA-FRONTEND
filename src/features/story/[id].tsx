...
export default function StoryViewPage() {
  ...
  const handleNext = () => {
    ...
    if (currentUserIndex < userStories.length - 1) {
      const nextUser = userStories[currentUserIndex + 1];
      const nextStory = nextUser.stories.find((s: any) => !viewedStories.has(s.id)) || nextUser.stories[0];
      router.replace(`/story/${nextStory.id}`);

      console.log("`/story/${nextStory.id}``/story/${nextStory.id}``/story/${nextStory.id}`", `/story/${nextStory.id}`);
    } else {
      // End of last user
      router.back();
    }
