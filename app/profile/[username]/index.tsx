...
export const TopHeader = memo<{ userName: string; theme: any; isOwnProfile: boolean }>(
    ({ userName, theme, isOwnProfile }) => {
        const navigation = useNavigation();
...
export const ProfileHeader = memo<{ theme: any; profile: any }>(
    ({ theme, profile }) => {
        const router = useRouter();
        ...
        const handleFollowingPress = useCallback(() => {
            router.push(`/profile/${profile.username}/followings`);
        }, [profile.username]);
...
export const UserInfo = memo<{
    ...
    return (
        <View style={styles.userInfo}>
            ...
            {user?.url ? (
                <Text style={styles.website} onPress={openUrl}>
                    {user.url}
                </Text>
            ) : null}
            ...
            {!isOwnProfile && (
                <View style={styles.actionButtons}>
                    <TouchableOpacity
                        onPress={onFollowToggle}
                        style={[styles.followButton, { backgroundColor: isFollowing ? theme.buttonSecondary || "#6c757d" : "#3498db" }]}
                    >
...
export const Tabs = memo<{ theme: any }>(({ theme }) => (
    <View style={[styles.tabs, { backgroundColor: theme.background }]}>
        <TouchableOpacity style={styles.tab}>
            <MaterialCommunityIcons name="view-grid-outline" size={22} color={theme.text} />
        </TouchableOpacity>
...
const VideoItem = memo<{
    ...
    index: number;
    onPressItem: (index: number) => void;
}>(({ videoUrl, image, id, username, index, onPressItem }) => {
    const player = videoUrl
        ? useVideoPlayer(videoUrl, (p) => {
            p.loop = true;
            p.muted = true;
...
export const PostGrid = memo<{ videos: any[]; username: string }>(
    ({ videos, username }) => {
        const router = useRouter();

        const handlePressVideo = useCallback((index: number) => {
            const video = videos[index];
            if (!video) return;
            router.push(`/p/${username}/${video.uuid}`);
        }, [videos, username]);
        ...
        const renderItem = useCallback(({ item, index }: { item: any; index: number }) => (
            <VideoItem
                ...
                onPressItem={handlePressVideo}
            />
        ), [username, handlePressVideo]);

        const keyExtractor = useCallback((item: any) => item.uuid, []);
...
export const ProfileScreen: React.FC = () => {
    ...
    }, [profile?.id, isFollowing, followMutation, unfollowMutation]);
