...
// //   // =====================================================
// //   // ✅ ONLY CAMERA UPLOAD — gallery completely removed
// //   // =====================================================
...
// //             {showImageOptions && (
// //               <View style={styles.imageOptionsContainer}>
// //                 <TouchableOpacity style={styles.imageOption} onPress={pickImageFromCamera}>
// //                   <Text style={styles.imageOptionText}>Camera</Text>
// //                 </TouchableOpacity>
...
// // import { GetCurrentUser, updateProfile } from "@/src/api/profile-api";
// // import { useAppTheme } from "@/src/constants/themeHelper";
// // import { useProfileStore } from "@/src/store/userProfileStore";
// // import { Ionicons } from "@expo/vector-icons";
...
// //   // const profileStore = profile?.userProfile;
// //   //  Get Zustand store
// //   const profileStore = useProfileStore();
...
// //               placeholderTextColor={theme.placeholder}
// //               value={profileData.username}
// //               onChangeText={(text) => setProfileData((prev) => ({ ...prev, username: text.toLocaleLowerCase().replace(/\s+/g, '').trim() }))}
// //             />
// //           </View>
...
function UserEditProfile() {
  ...
  // Update local state when currentUser data is fetched
  useEffect(() => {
    if (currentUser) {
      setProfileData({
        username: currentUser.username || "",
        name: currentUser.userProfile?.name || "",
        bio: currentUser.userProfile?.bio || "",
        ProfilePicture: currentUser.userProfile?.ProfilePicture || null,
        url: currentUser.userProfile?.url || "",
      });
      setCharacterCount({
        bio: currentUser.userProfile?.bio?.length || 0,
        url: currentUser.userProfile?.url?.length || 0,
      });
    }
  }, [currentUser]);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      queryClient.invalidateQueries({
        ...
        refetchType: "all" // Force refetch all userProfile queries
      });
      queryClient.invalidateQueries({ queryKey: ["stories"] });
        ...
        <ScrollView style={styles.container}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              onChangeText={(text) =>
                setProfileData((prev) => ({
                  ...prev,
                  username: text.toLocaleLowerCase().replace(/\s+/g, '').trim()
                }))
              }
            />
          </View>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              value={profileData.url}
              onChangeText={(text) => {
                setProfileData((prev) => ({ ...prev, url: text.trim() }));
                setCharacterCount((prev) => ({ ...prev, url: text.length }));
              }}
              maxLength={60}
              keyboardType="url"
